...
- **工具与 MCP 层**：哪些工具可被 Agent 直接调用；哪些命令在主机巡检场景中必须走白名单 / 黑名单 / 人工确认（参见 [主机巡检](/md/scenarios/host-forensics)）
- **沙箱层**：`flocks.json` 里的 `sandbox` 段控制运行隔离策略，包括作用范围、工作区访问权限、镜像、网络、内存、CPU 限制，参见 [Provider 与 API Services 配置](/md/integrations#provider-与-api-services-配置)
- **通道与外发层**：哪些通道有权接收结果；对外暴露时是否只开放 WebUI（参见 [公网暴露注意事项](/md/security#公网暴露注意事项)）
- **数据层**：高敏感数据是否应改走内网 / 私有模型（参见 [数据与脱敏说明](/md/security#数据与脱敏说明)）
...
1. 出问题先 `flocks status`、再 `flocks logs`
2. 涉及定时任务先看 [任务中心](/md/modules/tasks) 状态
3. 涉及工具或 MCP 先看 [工具清单](/md/modules/tools) 的配置与测试结果
4. 涉及模型错误走 [模型报错排查](/md/integrations#模型报错排查)
5. 仍无法定位再翻后端日志和 Workspace 产物
