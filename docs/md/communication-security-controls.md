...
Flocks 后端 API 与 WebUI 默认绑定 `127.0.0.1`，避免安装后直接暴露到全网。需要远程访问时，必须通过启动参数显式配置：
...
- 企业生产部署：通过 Nginx/网关统一暴露 WebUI，并将后端 API 置于同源 `/api` 反向代理后面。

WebUI 在后端绑定到非回环地址时，默认仍优先使用同源 `/api` 代理模式，避免浏览器绕过反代直接访问后端源站。CORS 配置也不会因为服务绑定 `0.0.0.0` 自动放开所有来源，远程部署需要显式配置允许的 origin。
...
- 后端 API 默认 `127.0.0.1:8000`；
- WebUI 默认 `127.0.0.1:5173`；
- 浏览器通过本机访问；
- 不对外暴露服务端口。
