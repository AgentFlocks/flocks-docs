...
如果设备支持 Syslog、Kafka 或 Webhook 推送，优先选择 **Workflow 接入**，把这类事件流接到 [Workflow 工作流](/md/modules/workflow#4-5-正式运行) 的发布 / 集成入口，而不是在这里创建 device 插件。可以前往 Workflow 页面的任意工作流，进入 **发布 / 集成** 页面查看并配置对应入口。
...
配置入口不在自定义设备插件表单中。请前往 [Workflow 工作流](/md/modules/workflow#4-5-正式运行) 页面，打开任意一个需要接收设备数据的 Workflow，在 **发布 / 集成** 页面查看并配置 Syslog、Kafka 等入口。
