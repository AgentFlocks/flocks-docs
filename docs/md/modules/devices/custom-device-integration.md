...
进入 **设备接入** 后，选择 **添加设备**，在厂商选择中进入 **自定义设备**。自定义设备当前主要有两条路径：

![选择自定义设备接入方式](../../../img/modules/devices/custom-access-mode.png)
...
如果设备支持 Syslog、Kafka 或 Webhook 推送，优先把这类事件流接到 [Workflow 工作流](/md/modules/workflow) 的 Integration 入口，而不是在这里创建 device 插件。

## API 接入流程

API 接入适合长期运行、定时任务、批量查询和需要稳定审计记录的场景。你需要向 Rex 提供 API 文档或接口说明，Rex 会先澄清缺失信息，再生成设备页可识别的 device 插件。

![API 接入时向 Rex 提供资料](../../../img/modules/devices/custom-api-rex-prompt.png)
...
![API device 插件配置表单](../../../img/modules/devices/custom-api-config.png)
...
![WebCLI 接入时向 Rex 提供资料](../../../img/modules/devices/custom-webcli-rex-prompt.png)
...
![WebCLI device 插件配置表单](../../../img/modules/devices/custom-webcli-config.png)
...
![自定义设备出现在设备接入列表](../../../img/modules/devices/device-list-custom.png)
