...
插件广场把 Skill、Agent、Tool、Workflow、WebUI 和场景套件放到同一个入口里管理，让用户可以从一个页面发现可用能力，并安装到本机插件目录后立即在平台中使用。
...
- **能力发现**：集中展示随 Flocks 版本发布到本地的 Skill、Agent、Tool、Workflow、WebUI 和场景套件。
...
| WebUI 和场景套件 | WebUI 插件会提供页面或页面组件；场景套件会把页面、工具、工作流等组合成完整场景。 |
...
WebUI 插件用于安装页面或页面组件。安装后，相关入口会出现在 Flocks WebUI 中，通常需要和 Tool、Workflow 或场景套件配合，才能形成完整业务能力。
...
场景套件用于把多个插件组合成一套可直接落地的业务场景。它通常会同时安装页面、工具、工作流或其他组件，适合一次性启用完整能力，而不是让用户逐个安装单插件。

典型用途：

- SOC 工作区场景套件。
- 告警运营能力组合。
- 面向特定安全场景的页面、工具和工作流打包安装。
...
- **类型**：Skill、Agent、Tool、Workflow、WebUI、场景套件。
...
- Workflow：尝试重新扫描工作流。
- WebUI：刷新页面组件和前端扩展入口。
- 场景套件：按套件内容安装对应页面、工具和工作流，并逐项刷新相关运行时。
...
- 场景套件卸载时，会移除该套件对应的页面、工具、工作流等组件。

### 5.4 安装 SOC 工作区场景套件

SOC 工作区场景套件是插件广场中的组合插件。进入 **Agent 工作室 → 插件广场**，按 **场景套件** 类型筛选，找到 **SOC 工作区场景套件** 后点击 **安装**。安装过程会逐项处理套件内组件，包括 SOC 工作区 UI、SOC 工作区操作工具、告警降噪工作流 `stream_alert_denoise` 和告警研判工作流 `stream_alert_triage`。

如果某个组件已经安装，安装进度中会显示为已跳过或已安装。套件安装完成后，列表状态会变为 **已安装**，操作按钮会切换为 **卸载**。

完整操作和截图见 [SOC 工作区场景套件](/md/scenarios/soc-workspace-suite)。
...
每个插件通常包含一个 `manifest.json`，声明插件类型、名称、版本、分类、标签、使用场景、依赖、权限、风险、入口文件和校验信息。WebUI 插件和场景套件等组合类能力通常会放在 `components/` 下。
...
- [SOC 工作区场景套件](/md/scenarios/soc-workspace-suite)：场景套件安装、配置和卸载示例。
