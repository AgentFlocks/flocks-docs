...
| [任务中心](/md/modules/tasks) | 周期任务的运行产物（中间数据、最终报告）通常落到 Workspace `outputs/` 下 |
...
```text
<workspace-root>/
├── outputs/         # 执行产物：报告、中间数据、测试数据
├── knowledge/       # 用户整理和维护的知识材料
└── ...
```

默认 Workspace 位于 `~/.flocks/workspace`，也可以通过 `FLOCKS_WORKSPACE_DIR` 环境变量覆盖。`outputs/` 用于保存 Agent、Workflow 和任务中心生成的报告、中间数据和测试数据；`knowledge/` 用于保存用户整理的知识材料。记忆系统的数据仍由运行时管理，通常位于 `~/.flocks/data/memory/`，WebUI 会提供只读视图。
...
- 新生成的产物会落到当前 Workspace 的 `outputs/`
...
- **终端查看**：进入 Workspace 根目录查看 `outputs/`
...
`outputs/` 不是日志，而是可再次使用的执行产物集合，包括测试数据、原始告警、JSON 报告和报表等。正因此：

- 告警研判里抓下来的原始告警 → 放入 `outputs/` 下的日期或任务目录，定时任务和后续会话可复用
- 工作流集成测试的测试数据 → 保留在 `outputs/` 下，回归验证时可直接取用
...
- 或者打包发布到团队内部库或兼容的技能/插件分发源，在其他项目中安装
