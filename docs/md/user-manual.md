...
> 安装启动与首次配置不再重复展开，统一放在 [Quick Start](/md/quick-start-install)，本页保留可离线导航与核心场景回执。
...
4. [模型配置](#四模型配置)
5. [通信与远程部署](#五通信与远程部署)
6. [通道配置（飞书 / 企微 / Slack / 微信 / WhatsApp / 钉钉 / Telegram / 邮件网关）](#六通道配置)
7. [运维与排障](#七运维与排障)
8. [升级方式](#八升级方式)
9. [安全与规范](#九安全与规范)
10. [场景演示](#十场景演示)
...
- 安装、启动、模型配置入口： [Quick Start](/md/quick-start-install)
- 访问控制与远程部署说明： [部署与配置](/md/communication)
- 运维与问题排查： [运维与排障](/md/operations)

### 2.2 任务优先级建议

- 第一次接触：先看 [推荐路径与安装要求](/md/quick-start-install)
- 本机交互优先：按 [Quick Start](/md/quick-start-install) 的一行命令路径
- 服务器/标准化部署：优先看 [Quick Start](/md/quick-start-install) 中的 Docker 路径
- 先跑通再探索：回到 [功能模块](/md/modules) 和 [场景实践](/md/scenarios)
...
需要监听所有网络接口时，可以使用 `flocks start --host 0.0.0.0`。此时应通过防火墙、安全组、反向代理和 TLS 限制访问范围。

### 5.2 Docker 远程访问

确认 `-p 5173:5173` 端口映射正确。“容器起来了但浏览器打不开”大多是端口映射或防火墙规则问题。

### 5.3 常见误区

1. 以为默认 `127.0.0.1` 也能让外部机器访问 ❌
2. 继续按旧版本额外开放 `8000`，而没有使用 `5173` 下的同源 `/api` 路径 ❌
...
- WebUI 与 API 共用同一个服务地址和端口，默认是 `127.0.0.1:5173`
- API 请求使用同源 `/api` 路径
- 非浏览器客户端调用 API 时必须携带 API Token

---

## 六、通道配置

当前支持：**飞书 / 企业微信 / Slack / 微信 / WhatsApp / 钉钉 / Telegram / 邮件网关**。

### 6.1 使用场景

- 研判结果主动推送至企微 / Slack / 微信 / WhatsApp / 钉钉 / Telegram 会话或邮箱
- 机器人在特定群、会话或邮件线程中接收消息
- 定时任务结果推送给值班或运营团队
...
- **Slack**：Slack App + Socket Mode + Bot Token / App Token
- **微信**：WebUI 扫码登录 iLink Bot，自动获取 `Token` / `Account ID`
- **WhatsApp**：WebUI 通过 Linked Devices 扫码配对，使用本地 Baileys bridge 连接 WhatsApp Web
- **邮件网关**：专用邮箱 + IMAP 收信 + SMTP 回信 + 发件人白名单
...
Agent 或 Workflow 分析
    ↓
通过企微 / Slack / 微信 / WhatsApp / 钉钉 / Telegram / 邮件网关等通道发送结果
```
...
1. 检查基础依赖（`uv`、`Node.js 22+`、`npm`、`agent-browser`）
2. 确认安装脚本完整执行
3. 确认 `flocks` 命令可用
4. 判断是否切换安装方式
...
| 监听地址恢复默认 | 升级跨越旧版服务模型、运行状态记录丢失或旧进程未正常退出 |
| 页面能开但功能异常 | 默认模型未恢复 / 前端缓存 / 升级未完成 / 旧进程未退 |

**恢复顺序**：`flocks status` → `flocks logs` → 确认旧进程退出 → 必要时执行 `flocks restart --host <ip> --port <port>` → 检查默认模型和通道。
...
- 只开放统一服务端口，不额外开放旧的后端端口
- 通过安全组 / 防火墙 / 反向代理限制访问范围
- 使用 TLS、账号登录和 API Token 保护远程访问
...
| `channels` | 消息通道 | 接飞书 / 企微 / Slack / 钉钉 / Telegram / WhatsApp / 邮件网关 |
...
| **Channel** | 把消息送到飞书 / 企微 / Slack / 钉钉 / Telegram / WhatsApp / 邮箱等外部触点的能力 |
