...
| 想快速启用 SOC 告警运营工作区 | SOC 工作区场景套件、告警降噪、告警研判 |
...
### [SOC 工作区场景套件](/md/scenarios/soc-workspace-suite)

一次性安装告警运营所需的 SOC 工作区 UI、SOC 工作区操作工具、告警降噪工作流和告警研判工作流。可以从新会话首页的 **告警运营** 入口触发安装，也可以在插件广场中按 **场景套件** 类型安装和卸载。

> 典型输出：SOC 工作区页面 · 告警降噪结果 · 结构化研判报告 · 可持续运营的工作流组合
...
相关：[项目概览](/md/overview) · [快速开始](/md/quick-start-install) · [功能模块](/md/modules) · [部署与配置](/md/communication) · [任务中心](/md/modules#任务中心) · [插件广场](/md/modules/flocks-hub)
