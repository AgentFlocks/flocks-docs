...
告警研判是 Flocks 最容易落地、使用频率也较高的场景。它的核心问题是：从安全设备获取告警后，如何在较短时间内为每条告警生成带证据链的结构化研判结论，并自动推送到对应的运营触点。
...
| 依赖项 | 要求 |
| --- | --- |
| 模型 | 已配置默认模型；推理型模型在复杂研判场景下效果更好。参考 [模型配置](/md/communication-models) |
...
抓取到的原始告警会写入 Workspace 的日期目录（如 `3-28/alerts.json`），而不是只保留在对话中。这是后续被定时任务复用的基础。
...
> 多群 / 多会话环境下，建议显式指定 `session ID`，避免消息发送到错误会话。详见 [企业微信通道配置 § 多群消息与 session ID](/md/channels/wecom#多群消息与-session-id)。
...
完成配置后，该链路即可按小时执行，并将结果按时发送到指定通道。

## 真实案例走读（NDR 告警 5 条研判）

以下为 NDR 告警研判演示中的关键节点：
...
| 2:42 | 识别出 "文件上传攻击 × 4"、"IP 109.x 重复出现"、"webshell + 内网横移" 等结论 | 基于证据链形成研判结论 |
...
企微 / 钉钉 / 飞书的通知会以文本摘要形式打包发送，便于值班组及时查看。

## 持续运行：从「做一次」到「常态化」

这是告警研判从单次处理走向持续运营的关键步骤。[任务中心](/md/modules/tasks) 可以把前面的一次性链路固化成：
...
| 告警量极大（每小时上万） | 分批处理：将 workflow 设计为按时间窗和告警类型切片 |
| 模型幻觉，研判结论不可信 | 在 Skill / Agent 的 system prompt 里明确要求"必须列出证据字段"，无证据时拒答 |
| 多群推送发送到错误群 | 显式指定 `session ID`，在 [企微通道配置](/md/channels/wecom#多群消息与-session-id) 中查询目标 ID |
