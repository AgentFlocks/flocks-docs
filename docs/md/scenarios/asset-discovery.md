...
该场景面向 ASM（攻击面管理）中的基础问题：给定一个域名或企业名，尽可能完整地梳理该组织暴露在互联网上的资产。

Flocks 在该场景中的价值不是提供另一个扫描器，而是将多个外部测绘源、企业内部 CMDB 和威胁情报放入同一个 Agent 上下文中，让资产数量和资产风险在同一份结果中呈现。
...
Flocks 的做法是让 Rex 串联整条链路：外部测绘、归类聚合、内部关联、风险打标和报告输出。
...
| [告警研判](/md/scenarios/alert-triage) | 告警里的目标资产反查是否属于本方资产 |
...
| 问题 | 处理 |
| --- | --- |
| 测绘结果噪声较大（包含非本方资产） | 以 Skill 沉淀判定规则：IP 归属、证书 subject、DNS SOA 等 |
| 一家测绘平台没覆盖到的资产 | 多源交叉；必要时补充 WHOIS + 证书透明度日志 |
| 影子资产被测绘出来但没人认领 | 在报告里专门开一个"无责任人"分区，推动流程补登 |
| 大型企业资产量过大（上万条） | 按业务 / 部门切片处理，避免一次性全量；必要时考虑分布式 Workflow |
