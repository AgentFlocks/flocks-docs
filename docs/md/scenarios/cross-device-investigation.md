...
跨设备深度调查的核心问题是：面对一条告警线索，如何让 Agent 在多设备之间联动取数，将碎片化数据拼接为完整攻击链。

Flocks 的做法是把"跨设备取数 → 时间窗对齐 → 攻击链路还原 → 溯源报告"封装为独立子 Agent，由主 Agent `Rex` 在研判过程中按需调用。子 Agent 同时具备 API 直连和浏览器操作两种取数能力，优先使用 API；无法通过 API 获取时，再降级到浏览器方案，该切换对上层流程保持透明。
...
这些都要求跨多个设备联合取数和时间轴对齐。人工完成一次跨设备调查通常需要较长时间，Flocks 可以显著缩短取数、对齐和汇总过程。

与「主机应急取证」的区别在于：主机取证是在单台机器内获取深度证据；跨设备调查是在外围多设备之间进行关联取证。两者通常配合使用：跨设备调查发现某台主机可疑后，再委派主机取证 Agent 上机调查。
...
| 依赖项 | 要求 |
| --- | --- |
| 模型 | 默认模型即可；推理型模型在时间线重建时效果更好——参考 [模型配置](/md/communication-models) |
...
所有原始数据会写入 Workspace 的日期目录（如 `3-28/cross/ndr.json`、`3-28/cross/hids.json`），便于后续复核和定时复用。
...
3. **因果推断**：识别事件先后关系、触发关系，以及初始入侵、横向移动、外联等关键节点
...
| --- | --- | --- |
| 触发 | 告警研判环节识别出 `10.10.x.x` 上有疑似 webshell 上传攻击 | 进入需要跨设备佐证的状态 |
...
跨设备调查本身是事件驱动的，不适合每小时无差别扫描。但以下衍生任务适合固化到 [任务中心](/md/modules/tasks)：
...
| 某个设备只有网页、没有 API | 启用浏览器工具作为降级方案，参考 [浏览器自动化与网页登录](/md/scenarios/browser-automation) |
| 并发查询影响设备稳定性 | 在 Agent / 工具层增加并发上限与退避重试，或按时间窗分批调查 |
| 报告太长难读 | 在 Agent 的 system prompt 里约束"时间线 + 证据矩阵 + 建议"三段式，超出就裁剪 |
| 想让 Agent 自动封禁 | 不建议。Flocks 的定位是研判 / 证据产出，封禁动作仍由人确认后交下游系统执行 |

---

相关：[场景总览](/md/scenarios) · [告警研判](/md/scenarios/alert-triage) · [主机巡检 / 应急取证](/md/scenarios/host-forensics) · [威胁情报与 IOC 研判](/md/scenarios/threat-intel) · [Agent 智能体](/md/modules/agents) · [工具清单](/md/modules/tools) · [任务中心](/md/modules/tasks)
