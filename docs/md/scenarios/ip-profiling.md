...
内网 IP 资产画像的核心问题是：告警中出现一个内网 IP 时，如何在较短时间内明确该资产是什么、归属哪个业务，以及近期是否存在异常行为。

Flocks 的做法是构建专门的 IP 画像 Agent，并发查询各设备与资产源，将 CMDB、HIDS、NDR、防火墙和历史告警进行聚合。该能力支持两种模式：快速查询用于告警研判中的摘要补充，深度画像用于事件调查中的完整报告。

## 场景简介

该场景看似粒度较小，但在真实运营中是降低误判的重要能力：
...
这些判断都依赖 IP 画像能力在研判过程中被快速调用。将其做成独立 Agent 或工具，而不是每次研判时由主 Agent 临时拼接，是为了保证稳定性、复用性和后续定时扩展能力。
...
| 业务知识库 | 可选。用于把业务背景写入画像，例如"这是支付网关" |
...
- 先从设备中筛选出近 24 小时有告警记录的 IP 子集，避免全量扫描
- 对每个 IP 调用快速画像
- 汇总成表，高风险单独标红
