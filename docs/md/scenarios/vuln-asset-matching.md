...
漏洞资产排查的核心问题是：新的高危 CVE 出现后，如何快速得到本企业内哪些资产受影响、哪些资产对外暴露，以及处置优先级如何排序的清单。

Flocks 的做法是构建漏洞资产排查 Agent：先获取漏洞情报，再并发进行多源资产匹配，随后交叉核查暴露面，最后输出按优先级排序的受影响资产清单。该流程可为应急响应提供直接可用的处置依据。

## 场景简介

漏洞应急的主要耗时通常不在补丁本身，而在定位：

- CVE 只给出"影响 X 组件 Y 版本"，需要映射为本企业中哪些机器运行了相关组件。
- HIDS 的软件清单、NDR 的流量指纹、资产扫描平台、CMDB 的组件版本分散在多个系统中。
- 还需要叠加该资产是否对外暴露、是否属于核心业务等信息来排序优先级。

人工完成一次排查通常需要较长时间，且容易遗漏。自动化后，可以在获取 CVE 编号后快速产出初始清单。
...
- CVE / CNVD 编号（最常见）
- 漏洞名称 + 受影响组件描述（半结构化，由 Agent 补齐细节）
...
| 通道 | 企微 / 钉钉 / 飞书任一已连通，用于通知应急响应组——参考 [通道配置](/md/communication-channels) |
...
| 希望 Agent 自动打补丁 | 不建议。Flocks 的定位是精准生成处置清单，补丁执行应由运维流程完成 |
