...
漏洞验证复现的核心问题是：已经拿到受影响资产清单后，如何自动确认哪些资产确实可被利用、哪些只是组件版本命中但不可达，以及哪些暂时无法验证。

Flocks 的做法是让漏洞验证 Agent 按漏洞类型选择合适的验证模式，在**授权 / 隔离**环境下自动执行验证动作，最终输出「存在 / 不存在 / 无法验证」三态结论 + 证据记录。

## 场景简介

漏洞资产排查能回答版本是否命中，但无法直接回答是否真实可利用。后者往往是应急响应中压缩处置范围的关键：

- 版本命中不等于可利用（可能有 WAF 拦截、组件未启用某功能）
- 不可利用的资产可以降级处置优先级
- 可利用的资产才应该进入"紧急补丁 / 临时下线"流程

因此，本场景的核心产出是每台资产的三态结论和对应证据。
...
> Agent 会记录每次尝试的原始请求 / 响应，而不只输出结论。这是漏洞验证场景最重要的可审计性保证。
...
漏洞验证 Agent 会根据 CVE 类型自动选择模式。若无法自动判定，会说明推断理由并请求用户确认。

### 步骤 3：授权边界确认

Agent 的第一步是确认目标在授权列表内。不在授权范围内的目标会被拒绝执行，并将原因记录到 Workspace。
