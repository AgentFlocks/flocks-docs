...
主机巡检场景的核心问题是：面对一台可能被攻陷的 Linux 主机，如何在不破坏环境的前提下，让 Agent 系统化地完成从基线收集到时间线重建的完整调查。

与告警研判不同，主机场景中的每一条命令都直接作用于真实机器，风险高于查询类动作。Flocks 的做法是在主 Agent Rex 下委派专门的主机巡检 Agent，并通过命令白名单、黑名单和人工逐条确认机制，将敏感操作与 AI 自动化执行边界隔离。
...
| 命令安全策略 | 白名单 / 黑名单 + 人工确认机制已在 Agent 中预设，通常无需额外配置 |
...
这套机制用于降低 Agent 直接操作主机带来的风险：

- 分析员无需担心 Agent 因错误判断执行破坏性命令
- 所有高危动作必须经过人工确认
...
- 主 Agent 的工作是将主机巡检任务交给专家 Agent
- 主机巡检 Agent 接管后，使用独立的 system prompt 和工具栈
- 两者共享整个会话上下文，但分析过程独立
...
这是关键控制点：用户可以拒绝任何不希望执行的命令，整个过程会保留操作痕迹。
...
| 3:33 | 还识别出其他爆破来源与成功登录来源 | 用户确认部分 key 属于正常运维行为，Agent 据此避免误判 |
...
| 问题 | 处理 |
| --- | --- |
| 希望 Agent 自动清理挖矿程序 | **不建议**。Flocks 的定位是研判 / 取证，处置动作仍应由人员确认后执行 |
| 灰区命令确认过多 | 在企业级 Skill 中扩展白名单，将组织公认安全的命令固化进去 |
