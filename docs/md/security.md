...
- 只开放 WebUI 与 API 共用的统一服务端口
- 浏览器通过同源 `/api` 路径调用 API
- 配合安全组、防火墙、TLS 或反向代理做访问控制
- 非浏览器客户端使用 API Token

不要沿用旧版本同时暴露 `5173` 和 `8000` 的方式。当前标准部署只需要统一服务端口，默认是 `5173`。

### 为什么不要把统一入口不受限制地暴露到公网

统一服务入口同时承载 WebUI 和 API。一旦直接暴露到公网，你面对的就不只是页面访问问题，而是：

- 更大的攻击面
- 未授权访问风险
- 接口被直接探测和调用的风险

因此，即便统一入口已经开启账号鉴权，也不建议不受限制地直接暴露到公网。应使用反向代理、TLS、可信网段或防火墙规则进一步收口。
